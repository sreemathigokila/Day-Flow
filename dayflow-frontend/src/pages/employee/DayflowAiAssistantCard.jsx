...
const DayflowAiAssistantCard = () => {
  ...
  return (
    <div className="bg-gradient-to-br from-slate-900 via-blue-950 to-purple-950 rounded-3xl p-6 text-white shadow-2xl border border-cyan-500/20 flex flex-col justify-between min-h-[460px]">
      {/* Header */}
      <div>
        <div className="flex items-center justify-between border-b border-slate-700/60 pb-4 mb-4">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-cyan-500 via-blue-600 to-purple-600 flex items-center justify-center text-white shadow-md shadow-cyan-500/20">
              <Bot className="w-6 h-6" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-lg font-extrabold text-white tracking-tight">Dayflow AI Assistant</h2>
              ...
              <p className="text-xs text-slate-300">Ask about your attendance, leave, payroll, or company HR policies...</p>
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {sampleQuestions.map((q, idx) => (
            <button
              ...
              className="text-xs bg-slate-800/80 hover:bg-cyan-950 border border-slate-700 hover:border-cyan-500/50 text-slate-300 hover:text-cyan-300 py-1 px-3 rounded-full transition-all"
            >
              {q}
            </button>
          ))}
            ...
            >
              <div
                className={`max-w-[85%] p-3 rounded-2xl text-xs leading-relaxed ${
                  msg.sender === 'user'
                    ? 'bg-gradient-to-r from-cyan-500 via-blue-600 to-purple-600 text-white rounded-br-none shadow-md'
      ...
      <div className="pt-4 border-t border-slate-700/60 mt-4">
        ...
        >
          <input
            ...
            className="w-full bg-slate-900/90 border border-slate-700 text-white placeholder-slate-500 rounded-2xl pl-4 pr-12 py-3 text-xs focus:outline-none focus:ring-2 focus:ring-cyan-500"
          />
          <button
            type="submit"
            disabled={loading || !prompt.trim()}
            className="absolute right-2 p-2 bg-gradient-to-r from-cyan-500 via-blue-600 to-purple-600 hover:opacity-90 disabled:opacity-40 text-white rounded-xl transition-all shadow-md"
          >
