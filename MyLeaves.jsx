...
import { FilePlus, FileCheck, Clock, CheckCircle2, XCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-extrabold text-slate-800 tracking-tight">My Leave Management</h1>
          <p className="text-xs text-slate-500">Apply for time off and view leave request statuses</p>
        </div>
        <button
          onClick={() => setShowModal(true)}
          className="py-2.5 px-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl text-xs flex items-center gap-2 shadow-md shadow-blue-500/20 transition-all"
        >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-slate-100 text-sm font-medium">
              {loading ? (
                <tr>
                  <td colSpan="6" className="py-8 text-center text-slate-400">Loading leave requests...</td>
                </tr>
              ) : leaves.length === 0 ? (
                <tr>
                  <td colSpan="6" className="py-8 text-center text-slate-400">No leave requests found.</td>
                </tr>
              ) : (
                leaves.map((l) => (
                  <tr key={l.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-4 px-6 text-slate-600 max-w-xs truncate">{l.remarks}</td>
            ...
            <form onSubmit={handleApply} className="space-y-3">
              ...
              <div className="flex gap-2 justify-end pt-2">
                ...
                <button
                  ...
                  className="py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl text-xs"
                >
